...
//     dd($perc);
#mysql://b466a12aaf6d48:663ba889@us-cdbr-east-06.cleardb.net/heroku_0b67c69e220a05c?reconnect=true
#/home/tauseef/Desktop/laravel_projects/cxai/app/Providers/AppServiceProvider.php
#db://user:password@host_url/database_name

/*DB_CONNECTION=mysql
DB_HOST=127.0.0.1
DB_PORT=3306
DB_DATABASE=cxai_web_db
DB_USERNAME=root
DB_PASSWORD=tauseef@121314*/ 

// DB_CONNECTION=mysql
// DB_HOST=us-cdbr-east-06.cleardb.net
// DB_PORT=3306
// DB_DATABASE=heroku_0b67c69e220a05c
// DB_USERNAME=b466a12aaf6d48
// DB_PASSWORD=663ba889
    ...
    if (!$myCart->isEmpty()) {
      ...
      return view('index', ['myJSON_recent' => $myJSON_recent,'recent_exists'=>  '1','last_id'=>$last_id,'search_type' => ""]);
     ...
     # return view('index');
      ...
      return view('index', ['recent_exists'=>  '0','last_id'=>0,'search_type' => ""]);
    }
  ...
  public function index_populate($myCart){
    ...
    if (!$myCart->isEmpty()) {
      ...
      return view('index', ['myJSON_recent' => $myJSON_recent,'recent_exists'=>  '1','last_id'=>$last_id]);
     ...
     # return view('index');
      ...
      return view('index', ['recent_exists'=>  '0','last_id'=>0]);
    }
...
$myCart =$myCart->unique();
        ...
        if (!$myCart->isEmpty()) {
          ...
          return view('index', ['myJSON_recent' => $myJSON_recent,'recent_exists'=>  '1','last_id'=>$last_id,'search_type' => $request->search]);
        }
        ...
        $query = DB::table('posts')->select('posts.post_id','posts.post','posts.user_id','posts.trigger_id','posts.posted_unix_time','posts.created_at','posts.updated_at','posts.post_title','posts.is_top_query','posts.total_responses','posts.is_answered','posts.user_id');
          ...
          $query = DB::table('posts')->select('posts.post_id','posts.post','posts.user_id','posts.trigger_id','posts.posted_unix_time','posts.created_at','posts.updated_at','posts.post_title','posts.is_top_query','posts.total_responses','posts.is_answered','posts.user_id');
            ...
            for ($x = 0; $x < count($search_item_array); $x++) {
              ...
              if($x == 0){
                  $query =$query->where('post_title', 'LIKE', '%'.$search_item_array[$x].'%')->where('post_title', 'not like', '%'.$search_item_array[$x].'%');
              }
              else{
                  $query = $query->orWhere('post_title', 'LIKE', '%'.$search_item_array[$x].'%')->where('post_title', 'not like', '%'.$search_item_array[$x].'%');
              }
        ...
        $myCart7 = DB::table('comments')
          ...
          $query = DB::table('comments')
            ...
            $myCart8 = $query->where('posts.is_approval', '>=', 0)->where('comments.is_approval', '>=', 0)->orderBy('posts.post_id', 'DESC')
                    ->distinct()->get();
          $myCart_temp = $myCart_temp1->merge($myCart8);
          $myCart_temp =$myCart_temp->unique(); 
...
############now deal with tags
        $myCart5 = DB::table('post_tags')
              ...
              ->select('posts.post_id','posts.post','posts.user_id','posts.trigger_id','posts.posted_unix_time','posts.created_at','posts.updated_at','posts.post_title','posts.is_top_query','posts.total_responses','posts.is_answered')->orderBy('posts.post_id', 'DESC')
              ->distinct()->get();
          $myCart_temp1 = $myCart_temp->merge($myCart5);
          $myCart_temp1 =$myCart_temp1->unique();
          ...
          $myCart_temp = $myCart_temp1->merge($myCart6);
          $myCart_temp =$myCart_temp->unique();                    
  ...
  public function get_triggers_ans(Request $request){
    $triggers =  DB::table('triggers')->where('trigger_id', '>', 0)->where('trigger_belong_to', '=', 'ans')->orWhere('trigger_belong_to', '=', 'all')->get();
    return $triggers;
      ...
      $search_type =  $tag_id .  " - Tag";
       ...
       $myCart = DB::table('post_tags')
        ...
        if (!$myCart->isEmpty()) {
          ...
          return view('index', ['myJSON_recent' => $myJSON_recent,'recent_exists'=>  '1','last_id'=>$last_id,'search_type' => $search_type ]);
