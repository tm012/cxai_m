...
@endphp
			...
			<div class="row">
				...
				<div class="col-md-12">
					<div class="crumbs">
						...
						<a href="#">Query</a> -->
						<span class="crumbs-span">/</span>
						<span class="current">Post No. {{$current_post_id}}</span>
	...
	@if(Auth::check())
		@php
			...
			$user_image_url = DB::table('users')->where('id', Auth::user()->id)->value('image_url');
		@endphp	
		...
		<div class="row">
			<div class="col-md-9">
				<article class="question single-question question-type-normal">
					<h2>
						<a style='white-space: pre-wrap;' class="post_title" style="cursor: none;" href="">{{$specific_query[0]->post_title}}</a>
					</h2>



					<a class="question-report" style="cursor: none;" >Post</a>
					<div style="display: none;"  class="question-type-main"><i class="icon-question-sign"></i>Question</div>
					<div class="question-inner">
						<div class="clearfix"></div>
						<div class="question-desc">
							<p style='white-space: pre-wrap;' class="post_body">{{$specific_query[0]->post}}</p>
						</div>
						...
						@php
						 	...
						 	$number_of_comments = DB::table('comments')->where('post_id', $current_post_id)->where('is_approval', '>=', 0)->count();
							$tags =  DB::table('tags')
							...
							$related_questions =  DB::table('posts')->where('is_approval', '>=', 0)->where('trigger_id', $specific_query[0]->trigger_id)->where('post_id', '!=' , $current_post_id)->limit(4)->get();
						...
						<span class="question-category"><a style="cursor: pointer;color: #1bbc9b;font-weight: bold;" data-toggle="modal" data-target="#myModalSeeTriggers" style=""><i class="icon-file"></i>Topic(s)</a></span>


						@if($specific_query[0]->reference_url !="")
							<span style="display: none;" class="question-category"><a href="{{$specific_query[0]->reference_url}}" target="_blank" style="cursor: pointer;color: #1bbc9b;font-weight: bold;"  style=""><i class="icon-external-link"></i>Main Reference </a></span>	

							<span style="display: none;" class="question-category"><a style="cursor: pointer;" data-toggle="modal" data-target="#myModalSeeReferances" style=""><i class="icon-file"></i>All References</a></span>						
	                	...
	                	@if(Auth::check())
							...
							@endif
						@endif
						<span class="question-category"><a style="cursor: pointer;" data-toggle="modal" data-target="#myModalEditParentList" style=""><i class="icon-list-ol"></i>Edit(s)</a></span>
				...
				<div   class="about-author clearfix">
				    <div style="display: none;"  class="author-image">
				    	<a href="#" original-title="admin" class="tooltip-n"><img alt="" src="http://placehold.it/60x60/FFF/444"></a>
				    </div>
				    <div style="display: none;" class="author-bio">
				        <p style="cursor: none;color: #1bbc9b;font-weight: bold;font-size: 20px;">Reference(s)</p>
				        @foreach($reference_list as $triggerForthisQuery=>$value)
							 <a style="font-size: 16px;"target="_blank" href='{{$value->reference_url}}' > {{$value->reference_url}}</a>
							 <br>
						@endforeach
				    </div>
				</div><!-- End about-author -->
				
				<div style="display: none;" id="related-posts">
							    ...
							    <li class="comment">
							        <div class="comment-body comment-body-answered clearfix"> 
							            ...
							            <div class="comment-text">
							                <div class="author clearfix">
												...
												@if(Auth::check())
									                @if((Auth::user()->id == $value->user_id))
									                    ...
									                    <span style="cursor: pointer;" onclick="editComment('{{$value->comment_id}}')"><i class="icon-edit"></i>Edit</span>

									                    <span style="display: none;cursor: pointer;" id="comment_edit_done_{{$value->comment_id}}" style="cursor: pointer;" onclick="myFunctionCommentEdited('{{$value->comment_id}}')"><i class="icon-check"></i>Done</span>
									                @endif
									            @endif

							                    <!-- <a  data-toggle="modal" data-post-id="{{$current_post_id}}" data-comment-id="{{$value->comment_id}}"  data-comment="{{$value->comment}}"data-target="#myModalCommentEdit"  style="cursor: pointer;" class="myModalCommentEdit" ><i class="icon-edit"></i>Edit</a> -->
							                ...
							                <p id="comment_trigger_{{$value->comment_id}}" style='font-weight: bold;color:black;' >{{$trigger_name}}</p>
							                <div class="text"><p style='white-space: pre-wrap;' id="comment_body_{{$value->comment_id}}">{{$value->comment}}</p><p  style='opacity: 0.5;display: none;' id='comment_body_instruction{{$value->comment_id}}'>Edit on the comment text box, after editing click Done button</p>
							        ...
							        @php
										$child_comments =  DB::table('comments')->where('post_id', '=', $current_post_id)->where('is_approval', '>=', 0)->where('parent_comment_id', '=', $value->comment_id)->get();
								        ...
								        <ul class="children">
								            <li class="comment">
												...
												@if(Auth::check())
									                ...
									                @if((Auth::user()->id == $value1->user_id)) 															
									                    <span style="cursor: pointer;" onclick="editComment('{{$value1->comment_id}}')"><i class="icon-edit"></i>Edit</span>

									                    <span style="display: none;cursor: pointer;" id="comment_edit_done_{{$value1->comment_id}}" style="cursor: pointer;" onclick="myFunctionCommentEdited('{{$value1->comment_id}}')"><i class="icon-check"></i>Done</span>
									                @endif
									            @endif
										                ...
										                <p id="comment_trigger_{{$value1->comment_id}}" style='font-weight: bold;color:black;'>{{$trigger_name}}</p>



								                        <div class="text"><p style='white-space: pre-wrap;'id="comment_body_{{$value1->comment_id}}">{{$value1->comment}}</p><p  style='opacity: 0.5;display: none;' id='comment_body_instruction{{$value1->comment_id}}'>Edit on the comment text box, after editing click Done button</p>
								                        </div>
										                @if($value1->is_verified == 1)
															<div class="question-answered question-answered-done"><i class="icon-ok"></i>Verified</div>
														@elseif($value1->is_verified == 0)
															<div style="color: red!important;" class="question-answered question-answered-done"><i class="icon-question-sign"></i>Not Verified Yet</div>
														@else
														<div style="color: red!important;" class="question-answered question-answered-done"><i class="icon-question-sign"></i>Check Fact</div>
														@endif
...
<input type="text" autocomplete="off"  onkeyup="getAnsTriggersParentComment()" name="selected_trigger_parent_comment" id="selected_trigger_parent_comment" placeholder="Write a Topic Name." >
...
<!-- 							<span class="styled-select">
							...
							<span class="form-description">Please mention an appropriate topic.</span>
						</p>				        
				...
				<div class="post-next-prev clearfix">
					@php
						$prev_record = DB::table('posts')->where('is_approval', '>=', 0)->where('post_id', '<', $current_post_id)->max('post_id');
						$next_record = DB::table('posts')->where('is_approval', '>=', 0)->where('post_id', '>', $current_post_id)->min('post_id');
			...
			<aside class="col-md-3 sidebar">
				...
				<div  class="widget ">
		            ...
		            <form id="search_form" method="GET" action="{{ action('QAController@search_index') }}">
		                      <input style="text-align: center;height: 100px;color:black;font-size:1.0em;width: 100%;font-size: 20px;" type="text" autocomplete="off" data-id='study' onkeyup="dummy_function(document.getElementById('search').value)" name="search" id="search" placeholder="Search...." class=" search_by_study ">
		                      <button style="display: none;" type="submit" class="search-submit" ></button>

		                      <p style="color: black;">The search will find entries whose tag/title/topic/author name will match your search term exactly. You can always disregard the suggestions that will be shown to you and search entries using a word/string of words. After typing, click the 'ENTER'/'SEARCH' button from the keyboard.</p>
		            </form>   
				...
				@php
					...
					$recentTwoPosts= DB::table('posts')->where('is_approval', '>=', 0)->limit(2)->orderBy('post_id', 'DESC')->get();
				@endphp
...
<div id="myModalEditParentList" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Previous Edit(s)</h4>
      </div>
							    ...
							    <div class="col-sm-6">
							    	...
							    	@foreach($posts_edit_lists as $posts_edit_list=>$value)	
							    		@if($value->type == "1")
											...
											<p style='white-space: pre-wrap;'>{{$value->edit}}</p>
...
function myFunctionCommentEdited(comment_id){

	comment_id_html = 'comment_body_' + comment_id;
	

	
	edited_post_body = document.getElementById(comment_id_html).innerHTML;
	if(edited_post_body !=""){
     ...
     $.ajax({
        ...
        success: function (data) {
          ...
          result = data;
          document.getElementById(comment_id_html).contentEditable = "false"; 
		  comment_id_html = 'comment_edit_done_' + comment_id;
		  document.getElementById(comment_id_html).style.display = "none"; 
			comment_id_html = 'comment_body_instruction' + comment_id;
			document.getElementById(comment_id_html).style.display = "none";
           ...
           error: function (jqXHR, status, err) {
              ...
              //location.reload(true);
              document.getElementById(comment_id_html).contentEditable = "true"; 
              alert("Try Again");
...
function editComment(comment_id){
	...
	comment_id_html = 'comment_edit_done_' + comment_id;
	document.getElementById(comment_id_html).style.display = "block"; 
	comment_id_html = 'comment_body_instruction' + comment_id;
	document.getElementById(comment_id_html).style.display = "block"; 	
         ...
         $.ajax({
            ...
            success: function (data) {
              ...
              //window.location.href = "/";
				...
				$('#add_additional_comment_parent' ).html(previous_val + "<li class='comment'><div class='comment-body comment-body-answered clearfix'><div class='avatar'><img alt='' src='"+current_user_image_url+"'></div><div class='comment-text'><div class='author clearfix'><div class='comment-author'><a>"+current_user_name+"</a></div><div class='comment-vote'><ul class='question-vote'><li><a style='cursor: default;' class='question-vote-up' title='Like'></a></li><li><a style='cursor: default;' class='question-vote-down' title='Dislike'></a></li></ul></div><span class='question-vote-result'>0</span><div class='comment-meta'><div class='date'><i class='icon-time'></i>"+get_date_time()+"</div> </div>  <a  data-toggle='modal' onclick='myFunction("+ result.toString()  +")' data-post-id='"+current_post_id.toString()+"' data-parent-id='"+new_comment_no.toString()+"' data-target='#myModalChildren'  style='cursor: pointer;' class='comment-reply myModalChildren' ><i class='icon-reply'></i>Reply</a><span style='cursor: pointer;' onclick='editComment("+result.toString() +")'><i class='icon-edit'></i>Edit</span><span style='display: none;cursor: pointer;' id='comment_edit_done_"+result.toString()+"' style='cursor: pointer;' onclick='myFunctionCommentEdited("+result.toString() +")'><i class='icon-check'></i>Done</span> </div><p style='font-weight: bold;color:black;' id='comment_trigger_"+result.toString()+"'>"+trigger_id+"</p><div class='text'><p style='white-space: pre-wrap;' id='comment_body_"+ result.toString() +"'>"+modal_comment+"</p><p  style='opacity: 0.5;display: none;' id='comment_body_instruction"+result.toString()+"'>Edit on the comment text box, after editing click Done button</p></div><div style='color: red!important;' class='question-answered question-answered-done'><i class='icon-question-sign'></i>Not Verified Yet</div> </div></div><div id='"+new_comment_id+"'>  </div></li>");
				new_comment_no = new_comment_no -1;
	        ...
	        success: function (data) {
	          //console.log("Success");
				...
				if(new_comment_new_post == 0){
					 ...
					 $( $child_id_additional ).html(previous_val + "<ul class='children'><li class='comment'><div class='comment-body clearfix'><div class='avatar'><img alt='' src='"+current_user_image_url+"'></div><div class='comment-text'><div class='author clearfix'><div class='comment-author'><a >"+current_user_name+"</a></div><div class='comment-vote'><ul class='question-vote'><li><a style='cursor: default;' class='question-vote-up' title='Like'></a></li><li><a style='cursor: default;' class='question-vote-down' title='Dislike'></a></li></ul></div><span class='question-vote-result'>0</span><div class='comment-meta'><div class='date'><i class='icon-time'></i>"+get_date_time()+"</div></div> <a style='display: none;'class='comment-reply' href='#'><i class='icon-reply'></i>Reply</a><span style='cursor: pointer;' onclick='editComment("+result.toString() +")'><i class='icon-edit'></i>Edit</span><span style='display: none;cursor: pointer;' id='comment_edit_done_"+result.toString()+"' style='cursor: pointer;' onclick='myFunctionCommentEdited("+result.toString() +")'><i class='icon-check'></i>Done</span></div><p style='font-weight: bold;color:black;' id='comment_trigger_"+result.toString()+"'>"+trigger_id+"</p><div class='text'><p style='white-space: pre-wrap;' id='comment_body_"+ result.toString() +"'>"+modal_comment+"</p><p  style='opacity: 0.5;display: none;' id='comment_body_instruction"+result.toString()+"'>Edit on the comment text box, after editing click Done button</p> </div> <div style='color: red!important;' class='question-answered question-answered-done'><i class='icon-question-sign'></i>Not Verified Yet</div> </div></div>  </li> </ul>");
